...
function handleStagger() {
    ...
    logMsg(
        "You're staggered and lose your footing!",
        "l-dmg"
    );
...
function handleReaction(actionType, payload) {
    ...
    if (actionType === "attack") {
        ...
        logMsg(
            "You seize the opening and strike before the monster can complete its move!",
            "l-good"
        );
...
function guard() {
    ...
    if (!hunt.pendingMove) {
        ...
        logMsg(
            "You settle into a ready stance, catching your breath.",
            "l-sys"
        );
...
function dodge() {
    ...
    logMsg(
        "You reposition, staying light on your feet.",
        "l-sys"
    );
...
function usePotion() {
    ...
    logMsg(
        "You down a potion, recovering 40 HP.",
        "l-sys"
    );
...
function handleFlee() {
    ...
    if (Math.random() < 0.75) {
        ...
        logMsg(
            "You break away and retreat from the hunting grounds.",
            "l-sys"
        );
